...
- `compose.candidate.yaml`은 `candidate-v1.0.11` 포인터, 후보 전용 volume 및
  ...
  `cardrag-worker-v114-candidate-state`와
  `cardrag-worker-v114-candidate-codex-home`으로 고정하고, MCP 후보는
  `cardrag-mcp-v114-candidate-state`만 사용합니다. 이 이름은 위 stable base
  기본값과 의도적으로 다르며 env로 재결합할 수 없습니다. Worker 후보는 원격 GC를
  하지 않고 MCP 후보는 기본 `127.0.0.1:18014`만 사용합니다. MCP overlay의 안전한
...
[v1.0.14 migration](../docs/V1_0_14_MIGRATION.md)의 canonical 명령으로 먼저 통과해야
...
사용하되 version/project/volume/port를 각각 `1.0.14`, `cardrag-v114-candidate`, v114 세
candidate volume, `18014`로 치환합니다. Incident destination 복구와 same-run 시작은
[v1.0.14 migration](../docs/V1_0_14_MIGRATION.md)을 먼저 통과해야 합니다.
...
따르며, 이번 v1.0.14 candidate에는 별도 cutover 승인이 아직 없습니다.
...
[v1.0.14 migration](../docs/V1_0_14_MIGRATION.md)을 따릅니다.
...
v1.0.14 runtime patch candidate가 v1.0.11 데이터 계약으로 합격하고 전환이 승인될 때까지
...
v1.0.14 후보가 합격하고 전환이 승인되면 v1.0.14 v4/v5 dual-reader MCP를 먼저 배치해
기존 v4를 제공하는지 확인하고, 이후에만 v1.0.14 Worker와 stable channel을 전환합니다.
