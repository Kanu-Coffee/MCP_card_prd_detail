...
# Fail whenever one drifts from its render source so maintenance services or a
# newly required flag cannot silently disappear from the distributed Stack.
for pair in \
...
do
    rendered=${pair%%:*}
    checked=${pair#*:}
    if ! cmp -s "$temporary_root/$rendered" \
        "$repository_root/deploy/portainer/$checked"; then
        echo "checked-in Portainer Stack is stale: $checked" >&2
        exit 1
    fi
done
CARDRAG_ADMIN_IMAGE=cardrag-admin@sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa \
...
for service in ("volume-init", "admin", "worker", "mcp"):
    ...
    for target in set(mounted) & set(expected_bind_sources):
        ...
        assert entry["bind"]["create_host_path"] is False, (service, target, entry)
...
assert migration["/var/lib/cardrag/objects"]["bind"]["create_host_path"] is False
...
assert all(
    item["bind"]["create_host_path"] is False
