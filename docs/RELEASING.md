...
현재 릴리스 검증기는 소프트웨어 **1.0.29**의 후보 증빙을 검증합니다.
...
뒤 source commit 다음의 **증빙만 추가하는 commit**에 `release-evidence/v1.0.29/`로
...
`APP_VERSION=1.0.29`, `VCS_REF=<commit>`,
...
annotated `v1.0.29` tag**에서 실행합니다. Tag 생성·공개 이미지 업로드는 검증을 마친 뒤의
...
Dispatch의 필수 입력은 `version=1.0.29`, `candidate_source_commit`,
