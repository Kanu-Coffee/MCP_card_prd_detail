...
AUDIT_SCHEMA: Final = "cardrag.legacy-v4-structure-audit.v1"
...
def validate_audit_artifact(payload: object) -> dict[str, Any]:
    ...
    if not isinstance(observed, dict) or set(observed) != set(METRIC_NAMES):
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        if arguments.database is not None:
            ...
            return 0
        artifact = load_audit_artifact(cast(Path, arguments.validate_artifact))
        validate_audit_artifact(artifact)
        return 0
