...
`cardrag.serving-db.v2` and `cardrag.serving-db.v3` generations. A background
...
the bounded Fasoo DRMONE `unsupported_drm` value; v2 remains readable so the MCP
can be upgraded before a v3 Worker publishes.
