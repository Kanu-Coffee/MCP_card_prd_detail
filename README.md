...
v1.0.5 Worker는 `cardrag.generation.v3`/`cardrag.serving-db.v3`를 게시하며,
...
v1.0.5는 희소 페이지 OCR 출력의 의미 없는 빈 줄을 정규화하는 호환 수정입니다.
애플리케이션 버전은 1.0.5로 올리되 native OCR의 processor contract는
...
desktop 공시 목록과 다시 결속한 뒤에만 PDF를 받습니다. v1.0.5의 운영 활성 목록은
`woori,kb,shinhan`입니다.

## 운영 이미지

v1.0.5 운영 배포에는 다음 두 고정 태그를 사용합니다.
...
1. 기존 `/opt/cardrag`을 보존하고 `/opt/cardrag-v1.0.5`에 immutable 배포 파일을
   준비합니다.
