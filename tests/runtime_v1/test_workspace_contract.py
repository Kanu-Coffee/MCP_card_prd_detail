...
def test_default_deployment_has_only_worker_and_mcp() -> None:
    ...
    assert re.search(r"^FROM \${WOLFI_BASE_IMAGE} AS worker$", dockerfile, re.MULTILINE)
...
def test_candidate_deployment_isolated_from_stable_runtime() -> None:
    ...
    assert "cardrag-worker-v122-candidate-state" in worker
    assert "cardrag-worker-v122-candidate-codex-home" in worker
    assert "${CARDRAG_CANDIDATE_MCP_STATE_VOLUME:-cardrag-mcp-v122-candidate-state}" in mcp
    assert "CARDRAG_CANDIDATE_MCP_PUBLISHED_PORT:-18022" in mcp
    assert "target: /mnt/cardrag-seed-state" in cache_seed
    assert "read_only: true" in cache_seed
    assert "external: true" in cache_seed
    assert "CARDRAG_SEED_WORKER_STATE_VOLUME:?" in cache_seed
    assert "nocopy: true" in cache_seed
    ...
    # Remote collection and its authorization both stay off by default.
    ...
    assert "cardrag-worker-v122-state" in worker_base
    assert "cardrag-worker-v122-codex-home" in worker_base
    assert "cardrag-mcp-v122-state" in mcp_base
    assert "cardrag-worker-v122-candidate-state" not in worker_base
    assert "cardrag-worker-v122-candidate-codex-home" not in worker_base
    assert "cardrag-mcp-v122-candidate-state" not in mcp_base


def test_candidate_mcp_retry_can_isolate_project_and_state_volume() -> None:
    ...
    environment.update(
        {
            ...
            "CARDRAG_CANDIDATE_MCP_PUBLIC_BASE_URL": "http://127.0.0.1:18022",
    ...
    assert default_config["name"] == "cardrag-v122-candidate"
    assert default_config["volumes"]["mcp-state"]["name"] == ("cardrag-mcp-v122-candidate-state")

    retry_environment = environment | {
        "COMPOSE_PROJECT_NAME": "cardrag-v122-candidate-retry",
        "CARDRAG_CANDIDATE_MCP_STATE_VOLUME": ("cardrag-mcp-v122-candidate-retry-state"),
    }
    retry_config = render(retry_environment)
    assert retry_config["name"] == "cardrag-v122-candidate-retry"
    assert retry_config["volumes"]["mcp-state"]["name"] == ("cardrag-mcp-v122-candidate-retry-state")
...
def test_candidate_capacity_and_issuer_contract_reject_ambient_overrides() -> None:
    ...
    assert worker_config["volumes"]["worker-state"]["name"] == ("cardrag-worker-v122-candidate-state")
    assert worker_config["volumes"]["codex-home"]["name"] == ("cardrag-worker-v122-candidate-codex-home")
    ...
    assert mcp_config["volumes"]["mcp-state"]["name"] == ("cardrag-mcp-v122-candidate-state")
    for name, expected in capacity.items():
